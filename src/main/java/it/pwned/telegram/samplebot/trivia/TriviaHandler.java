...
public class TriviaHandler implements UpdateHandler, Runnable {
	...
	private static final InlineKeyboardMarkup highscoreKb;
	static {
		...
		InlineKeyboardButton btnHs = new InlineKeyboardButton("Show top Highscores", null, "HS", null, null, null);
		kb.addButton(btnHs, 0);
		
		highscoreKb = kb.build();
	}
	...
	private void handleInlineQuery(InlineQuery query) {
		...
		for (int i = 0; i < 3; i++) {
			log.trace(String.format("Generating option: %s", categories.get(categoryIndexes.get(i))));
			results
	...
	private void handleChosenResult(ChosenInlineResult chosenInlineResult) {
		...
		try {
			...
			log.trace(String.format("Fetching question with category [%s] and difficulty [%s]", category, difficulty));
			...
			if ("FAIL".equals(callbackQuery.data)) {
				...
				if (count > 0) {
					...
					client.call(new EditMessageText(callbackQuery.inlineMessageId, HtmlUtils.htmlUnescape(newText),
							ParseMode.HTML, null, highscoreKb));

					jdbc.update("UPDATE PUBLIC.QUESTION_DATA SET WINNING_USER_ID = ?, CHAT_INSTANCE = ? WHERE QUESTION_ID = ? ;",
							new Object[] { callbackQuery.from.id, callbackQuery.chatInstance, callbackQuery.inlineMessageId });
	...
	private void showHighscore(CallbackQuery callbackQuery) {
